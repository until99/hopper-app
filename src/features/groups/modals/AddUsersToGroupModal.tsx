...
export default function AddUsersToGroupModal({
    ...
    const fetchData = async () => {
        ...
        setError(null);
        try {
            const [usersData, groupUsersData] = await Promise.all([
                usersService.fetchUsers(1, 100),
                groupsService.fetchGroupUsers(groupId)
            ]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            for (const userId of usersArray) {
                await groupsService.addUserToGroup(groupId, userId);
            }
            onUsersAdded();
            onClose();
        } catch (err) {
